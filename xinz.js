...
xinz.on('message-new', async(qul) => {
    try {
		...
		//const isCpref = bodi.startsWith(cprefix)
		const q = args.join(" ")
        ...
        if (isGroup){
            ...
            aqul.sendFakeStatus(from, `Waalaikumsalam, Halo Kak, Untuk Memulai bot silahkan ketik ${prefix}help`, fake)
        }
    }
        if (isGroup){
        	if (chats.toLowerCase() === 'Assalamualaikum') {
...
/*		if (itsMe){
	...
	/*if (isCpref && !isGroup) {console.log(color('[Custom]'), color(moment(qul.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${tai} [${args.length}]`))}
        if (isCpref && isGroup) {console.log(color('[Custom]'), color(moment(qul.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${tai} [${args.length}]`), 'from', color(xinz.user.name), 'in', color(groupName))}
        switch (tai) {
			case 'Apakah':
			case 'apakah':
				buriq = arg.endsWith('?')
